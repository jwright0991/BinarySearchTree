...
def treeCountOddGE(root, start):
    ...
    while(node != None):
        if (node.data >= start):
            if not(node.isEven):
                countOdd +=1
            if(node.rightChild != None and not(node.rightChild.isEvenSum)):
                countOdd +=1
            node = node.leftChild
        else:
...
def treeCountOddLE(root, end):
    ...
    while(node != None):
        if (node.data <= end):
            if not (node.isEven):
               countOdd +=1
            if(node.leftChild != None and not(node.leftChild.isEvenSum)):
                countOdd += 1
            node = node.rightChild
        else:
            node = node.leftChild
...
#@param end
#   the largest value that a node can contain for it to be counted
def treeCountOddInRange(tree, start, end):
        if tree.root == None:
            return 0
...
if len(sys.argv) == 3:
    ...
    for line in dataRanges:
        ...
        if treeCountOddInRange(tree,start,end) % 2 == 0:
            result = "even sum"
        else:
            result = "odd sum"
        #print the results
